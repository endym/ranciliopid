...
// text align
#define ALIGN_CENTER(CString) ((DISPLAY_WIDTH - (u8g2.getStrWidth(CString))) / 2)
#define ALIGN_RIGHT(CString) (DISPLAY_WIDTH - u8g2.getStrWidth(CString))
#define ALIGN_LEFT 0
...
static void cbMenuDynParaPidOn(uint8_t line);
...
// FIXME extern SysPara<double> sysParaBrewSetPoint;
...
LCDML_addAdvanced(1, LCDML_0_1, 1, NULL, "", cbMenuDynParaPidOn, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(2, LCDML_0_1, 2, NULL, "", cbMenuDynParaBrewSetPoint, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(3, LCDML_0_1, 3, NULL, "", cbMenuDynParaSteamSetPoint, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(4, LCDML_0_1, 4, NULL, "", cbMenuDynParaPidKp, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(5, LCDML_0_1, 5, NULL, "", cbMenuDynParaPidTn, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(6, LCDML_0_1, 6, NULL, "", cbMenuDynParaPidTv, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(7, LCDML_0_1, 7, NULL, "Cold Start", NULL, 0, _LCDML_TYPE_default);
LCDML_addAdvanced(8, LCDML_0_1_7, 1, NULL, "", cbMenuDynParaPidKpStart, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(9, LCDML_0_1_7, 2, NULL, "", cbMenuDynParaPidTnStart, 0, _LCDML_TYPE_dynParam);
LCDML_add(10, LCDML_0_1_7, 3, "Back", cbMenuBack);
LCDML_addAdvanced(11, LCDML_0_1, 8, NULL, "Brew Detection", NULL, 0, _LCDML_TYPE_default);
LCDML_addAdvanced(12, LCDML_0_1_8, 1, NULL, "", cbMenuDynParaBdThreshold, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(13, LCDML_0_1_8, 2, NULL, "", cbMenuDynParaBrewSwTimer, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(14, LCDML_0_1_8, 3, NULL, "", cbMenuDynParaPidKpBd, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(15, LCDML_0_1_8, 4, NULL, "", cbMenuDynParaPidTnBd, 0, _LCDML_TYPE_dynParam);
LCDML_addAdvanced(16, LCDML_0_1_8, 5, NULL, "", cbMenuDynParaPidTvBd, 0, _LCDML_TYPE_dynParam);
LCDML_add(17, LCDML_0_1_8, 6, "Back", cbMenuBack);
LCDML_add(18, LCDML_0_1, 9, "Save", cbMenuSaveSettings);
LCDML_add(19, LCDML_0_1, 10, "Back", cbMenuBack);
LCDML_add(20, LCDML_0, 2, "Information", cbMenuInformation);
LCDML_add(21, LCDML_0, 3, "Exit", cbMenuExit);
    ...
    for (btnIdx = 0; btnIdx < BTN_TYPE__LAST_ENUM; btnIdx++) {
        ...
        if (currentButtonStatus != lastButtonStatus[btnIdx]) {
            ...
            case BTN_STATUS_PRESSED:
                Serial.printf("%s(): button %u pressed", __FUNCTION__, btnIdx);
                // do action only on next release, because it may become "hold" status
                break;
            case BTN_STATUS_HOLD:
                Serial.printf("%s(): button %u hold", __FUNCTION__, btnIdx);
                LCDML.BT_enter();
                break;
            case BTN_STATUS_RELEASED:
                Serial.printf("%s(): button %u released", __FUNCTION__, btnIdx);
                ...
                break;
            default:
                Serial.printf("%s(): button %u unknown state", __FUNCTION__, btnIdx);
                break;
            }
...
static void lcdml_menu_display(void)
{
    ...
    if ((tmp = LCDML.MENU_getDisplayedObj()) != NULL)
    {
        ...
        if (LCDML_TITLE_ROWS > 0)
        {
            ...
            char content_text[DISPLAY_WIDTH/u8g2.getMaxCharWidth() + 1];    // title font has different font width
            if (LCDML.MENU_getLayer() == 0)   // root menu?
            {                                 // yes, use fixed text...
                strncpy(content_text, getMachineName(), sizeof(content_text) - 1);
...
static void cbMenuExit(uint8_t param)
{
    if (LCDML.FUNC_setup()) {
        ...
        Serial.printf("%s(): exit display menu", __FUNCTION__);
        // LCDML.MENU_goRoot();
 ...
 *
 * @param[in]     line      - screen line of this parameter
 * @param[in,out] sysPara   - global system parameter
 * @param[out]    dispValue - parameter value to display
 * @param[in]     stepSize  - value to dec/add per change
 */
template <typename T>
static void handleDynPara(uint8_t line, menu_dyn_para<T> &dynMenuPara, T stepSize)
{
    ...
    // sysParaCopy = sysPara;
    dynMenuPara.dispValue = dynMenuPara.sysPara.get();
    ...
    {                                  // yes...
        ...
        {                              // yes...
            ...
            {                          // yes, switch between scroll/edit mode...
                ...
                // parameter value instead of jump to prev/next menu item. Therefore the
    ...
    if (dynMenuPara.isEditMode && (line == curLine)) // edit mode and cursor at edit line?
    ...
    else { // no, normal navigation mode...
        // clear line...
        u8g2.setDrawColor(0);
        u8g2.drawBox(0, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT, DISPLAY_WIDTH, DISPLAY_FONT_HEIGHT);
        u8g2.setDrawColor(1);
...
/**
 * @brief Callback for setting parameter "PID on/off".
 *
 * @param line - display line of this parameter
 */
static void cbMenuDynParaPidOn(uint8_t line)
{
    static menu_dyn_para<uint8_t> dynMenuPara;
    char lineText[LCDML_COLS + 1];

    dynMenuPara.sysPara = sysParaPidOn;
    handleDynPara(line, dynMenuPara, (uint8_t)1);
    snprintf(lineText, sizeof(lineText), "PID %12s", (dynMenuPara.dispValue==1)?"on":"off");
...
static void cbMenuDynParaBrewSetPoint(uint8_t line)
{
    static menu_dyn_para<double> dynMenuPara;
    char lineText[LCDML_COLS + 1];

    dynMenuPara.sysPara = sysParaBrewSetPoint;
    handleDynPara(line, dynMenuPara, 0.5);
    snprintf(lineText, sizeof(lineText), "Brew Temp. %5.1f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
    u8g2.setDrawColor(1);
}
...
static void cbMenuDynParaSteamSetPoint(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaSteamSetPoint;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "Steam Temp.  %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
    u8g2.setDrawColor(1);
}
...
static void cbMenuDynParaPidKpStart(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidKpStart;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Kp       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
    u8g2.setDrawColor(1);
}
...
static void cbMenuDynParaPidTnStart(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidTnStart;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Tn       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
    u8g2.setDrawColor(1);
}
...
static void cbMenuDynParaPidKp(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidKpReg;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Kp       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
...
static void cbMenuDynParaPidTn(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidTnReg;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Tn       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
...
static void cbMenuDynParaPidTv(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidTvReg;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Tv       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
    u8g2.setDrawColor(1);
}
...
static void cbMenuDynParaPidKpBd(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidKpBd;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Kp       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
...
static void cbMenuDynParaPidTnBd(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidTnBd;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Tn       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
...
static void cbMenuDynParaPidTvBd(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaPidTvBd;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "PID Tv       %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
...
static void cbMenuDynParaBdThreshold(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaBrewThresh;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "Threshold    %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
    u8g2.setDrawColor(1);
}
...
static void cbMenuDynParaBrewSwTimer(uint8_t line)
{
    ...
    dynMenuPara.sysPara = sysParaBrewSwTimer;
    handleDynPara(line, dynMenuPara, 1.0);
    snprintf(lineText, sizeof(lineText), "SW Timer     %3.0f", dynMenuPara.dispValue);
    u8g2.setCursor(LCDML_TEXT_INDENTING, (LCDML_TITLE_ROWS + line) * DISPLAY_FONT_HEIGHT);
    u8g2.print(lineText);
    u8g2.setDrawColor(1);
}
...
void displaymenuSetup(void)
{
    ...
    Serial.printf("%s(): init buttons", __FUNCTION__);
    ...
    // menu
    Serial.printf("%s(): init menu", __FUNCTION__);
...
bool displaymenuIsEnableEvent(void)
{
    ...
    {                                 // yes, check for button click...
        ...
        {                       // yes, enable menu...
            Serial.printf("%s(): enable menu", __FUNCTION__);
            buttonPressed = false;
            isMenuEnabled = true;
